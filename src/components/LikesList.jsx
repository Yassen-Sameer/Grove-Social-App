...
import { X, UserPlus, Loader, UserCheck } from "lucide-react";
...
export default function LikesList() {
  ...
  async function getLikers() {
    const options = {
      method: "GET",
      url: `https://route-posts.routemisr.com/posts/${id}/likes?page=1&limit=20`,
      headers: {
    ...
    try {
      ...
      setLikers(data.data.likes);
      setLoading(false);
    } catch (error) {
      toast.error("failed to get Likers");
      setLoading(false);
    }
  }
            ...
            {loading ? (
              ...
              <>
                {likers.map((liker) => (
                  ...
                  >
                    <Link
                      to={`${userInfo.id === liker.id ? `/profile` : `/user/${liker.id}`}`}
                    >
